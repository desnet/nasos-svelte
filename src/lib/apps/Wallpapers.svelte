	...
	<div class="preview-pane">
		...
		<div class="preview-actions">
			{#if preview && preview.id !== wallpaperStore.current.id}
				<button class="btn-apply" onclick={() => apply(preview!)}>
					✓ Применить
				</button>
				<button class="btn-cancel" onclick={() => (preview = null)}>
					Отмена
				</button>
			{:else}
...
<style>
	...
	.preview-name { font-weight: 700; font-size: 14px; color: #222; }
	.preview-cat {
		font-size: 11px;
		color: #888;
		background: #eef0f8;
...
.grid {
	...
	.thumb:hover { transform: scale(1.04); border-color: #aac4e8; }
	.thumb.previewing { border-color: #4a90d9; box-shadow: 0 0 0 2px rgba(74,144,217,0.3); }
	.thumb.current { border-color: #2a70b9; }
	...
	.thumb-badge.current { background: #4a90d9; color: white; }
