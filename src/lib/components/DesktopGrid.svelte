<script lang="ts">
  ...
  $effect(() => {
    ...
    function onExternalMove(e: MouseEvent) {
      ...
      const col = Math.max(
        1,
        Math.min(Math.floor((e.clientX - rect.left - launcherDrag.mouseOffX) / cellW) + 1, cols)
      );
      const row = Math.max(
        1,
        Math.min(Math.floor((e.clientY - rect.top - launcherDrag.mouseOffY) / cellH) + 1, rows)
      );
  ...
  function onMousemove(e: MouseEvent) {
    ...
    const ghostCol = Math.max(
      1,
      Math.min(Math.floor(cellLeft / cellW) + 1, cols - drag.item.colSpan + 1)
    );
    const ghostRow = Math.max(
      1,
      Math.min(Math.floor(cellTop / cellH) + 1, rows - drag.item.rowSpan + 1)
    );
