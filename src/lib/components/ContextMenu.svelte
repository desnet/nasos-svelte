...
<div class="ctx-overlay" onclick={onclose} oncontextmenu={(e) => { e.preventDefault(); onclose() }}></div>

<div class="ctx-menu" style="left: {menuX}px; top: {menuY}px" bind:clientWidth={menuW} bind:clientHeight={menuH}>
...
<style>
	...
	.ctx-item:hover { background: rgba(74, 144, 217, 0.3); }
	.ctx-item.danger:hover { background: rgba(220, 50, 50, 0.35); }

	.ctx-icon { font-size: 15px; width: 20px; text-align: center; }
